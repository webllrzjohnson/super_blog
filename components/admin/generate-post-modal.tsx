      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {wordCount ?? quality.wordCount} words · {blocking.length} blocking
...
export function GeneratePostModal({
  ...
  const selectedPreset = AI_PROMPT_PRESETS.find(
    (preset) => preset.id === promptPreset,
  );
    ...
    try {
      ...
      const details = [
        (data.post?.slug ?? data.slug)
          ? `Slug: ${data.post?.slug ?? data.slug}`
          : null,
        data.wordCount ? `${data.wordCount} words` : null,
        data.providerAttempts?.length
          ? data.providerAttempts.join(" → ")
          : null,
        data.warnings?.length
          ? `${data.warnings.length} quality warning(s)`
          : null,
      ].filter(Boolean);
        ...
        <div className="mb-4 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Generate a preview first, review quality warnings, then save it as
              a draft.
            </p>
          </div>
          <button
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                value={promptPreset}
                onChange={(e) =>
                  setPromptPreset(e.target.value as typeof promptPreset)
                }
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              >
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Preview saves as draft; use the editor to publish or schedule
                after review.
              </p>
            </div>
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">
                Featured image for direct save
              </label>
              <input
                type="file"
                accept="image/*"
              ...
              />
              <p className="mt-1 text-xs text-muted-foreground">
                Optional. If empty, the AI workflow generates a featured image
                with the saved OpenAI key.
              </p>
              {featuredImage && (
                <p className="mt-1 text-xs text-muted-foreground">
                  {featuredImage.name}
              ...
              <div className="flex min-h-[360px] items-center justify-center rounded-lg border border-dashed border-border bg-muted/20 p-8 text-center">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Generate a draft preview to inspect title, excerpt, tags,
                    and quality warnings before saving.
                  </p>
                </div>
              </div>
            ) : (
              <>
                ...
                <article className="rounded-lg border border-border bg-background p-5 shadow-sm">
                  {preview.post.featuredImage ? (
                    <div className="relative mb-4 aspect-[3/2] overflow-hidden rounded-lg bg-muted">
                      <Image
                        ...
                        alt={
                          preview.post.featuredImageAlt || preview.post.title
                        }
                        width={768}
                        height={512}
                  ...
                  <div className="mb-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                    <span>
                      {preview.model ? AI_PROVIDER_LABELS[preview.model] : "AI"}
                    </span>

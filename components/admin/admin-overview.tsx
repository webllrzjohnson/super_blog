            ...
            {posts.map((post) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className={`rounded-full px-2 py-1 text-xs ${statusClass(post.status)}`}
                  >
                    {post.status}
                  </span>
                </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-border bg-card p-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Review content health, keep the schedule filled, and jump into the
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          helper={
            stats.staleDrafts
              ? `${stats.staleDrafts} stale draft(s)`
              : "Ready for review"
          }
          icon={FileText}
        />
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_0.8fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              What needs attention before the next post goes live.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {actionItems.length === 0 ? (
              <div className="rounded-lg border border-green-200 bg-green-50 p-4 text-sm text-green-800 dark:border-green-900/40 dark:bg-green-950/30 dark:text-green-300">
                Content pipeline looks healthy. Keep one draft and one scheduled
                post ready.
              </div>
            ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Published and scheduled posts by day.
            </CardDescription>
          </CardHeader>
          <CardContent>

...
export function SettingsAi({ initialValue }: SettingsAiProps) {
  const [formData, setFormData] = useState<AiSettings>({
    providerOrder: initialValue?.providerOrder ?? [
      ...DEFAULT_AI_PROVIDER_ORDER,
    ],
    ...
    userMessageTemplate:
      initialValue?.userMessageTemplate ??
      defaultAiSettings.userMessageTemplate,
    groqUserMessageTemplate:
      initialValue?.groqUserMessageTemplate ??
      defaultAiSettings.groqUserMessageTemplate,
    imagePromptTemplate:
      initialValue?.imagePromptTemplate ??
      defaultAiSettings.imagePromptTemplate,
  });
  const [isSaving, setIsSaving] = useState(false);
  const [apiKeyStatus, setApiKeyStatus] = useState<ApiKeyProviderStatus[]>([]);
  const [apiKeyDraft, setApiKeyDraft] = useState<
    Record<ApiKeyProviderStatus["provider"], string>
  >({
    anthropic: "",
    openai: "",
    groq: "",
  });
  const [apiKeyClear, setApiKeyClear] = useState<
    Record<ApiKeyProviderStatus["provider"], boolean>
  >({
    anthropic: false,
    openai: false,
    groq: false,
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add or rotate Claude, OpenAI, and Groq keys without editing
            environment variables. Saved keys are masked after saving; blank
            fields keep the current key.
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            These model IDs are used for post and image generation. API keys can
            be saved in the card above or kept in environment variables.
          </CardDescription>
        </CardHeader>
          ...
          <div className="rounded-lg border border-border bg-muted/30 p-4 space-y-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Generation tries providers in this order. Failed API calls,
                empty output, or failed quality checks automatically fall back
                ...
                <div key={`${provider}-${index}`} className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateProviderOrder(
                        index,
                        event.target.value as AiTextProvider,
                      )
                    }
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  updateField("claudeModel", event.target.value)
                }
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  updateField("groqModel", event.target.value)
                }
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  updateField("imageModel", event.target.value)
                }
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Claude and OpenAI use the full system prompt. Groq uses the shorter
            fallback. Keep the ---JSON--- / ---CONTENT--- / ---END--- response
            format in system prompts or generation will fail.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(event) =>
                updateField("claudeSystemPrompt", event.target.value)
              }
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(event) =>
                updateField("groqSystemPrompt", event.target.value)
              }
              className="min-h-[200px] font-mono text-xs"
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="userMessageTemplate">
              Claude user message template
            </Label>
            <Textarea
              id="userMessageTemplate"
              value={formData.userMessageTemplate}
              onChange={(event) =>
                updateField("userMessageTemplate", event.target.value)
              }
              className="min-h-[160px] font-mono text-xs"
            />
            <p className="text-sm text-muted-foreground">
              Placeholders: {"{{topic}}"}, {"{{context}}"}, {"{{schedule}}"},{" "}
              {"{{recentPosts}}"}
            </p>
          </div>
          <div className="space-y-2">
            <Label htmlFor="groqUserMessageTemplate">
              Groq user message template
            </Label>
            <Textarea
              id="groqUserMessageTemplate"
              value={formData.groqUserMessageTemplate}
              onChange={(event) =>
                updateField("groqUserMessageTemplate", event.target.value)
              }
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Used when generating featured images without an upload. A random
            Toronto scene is picked for {"{{setting}}"} on each generation.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          ...
          <Textarea
            ...
            onChange={(event) =>
              updateField("imagePromptTemplate", event.target.value)
            }
            className="min-h-[160px] font-mono text-xs"
          />
          <p className="text-sm text-muted-foreground">

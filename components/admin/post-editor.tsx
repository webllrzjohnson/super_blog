    ...
    >
      ...
      {hasIssues && (
        <p className="text-xs font-medium text-foreground">
          {errors.length} blocking · {warnings.length} warning
          {warnings.length === 1 ? "" : "s"}
        </p>
      )}
...
export function PostEditor({
  ...
  const featuredAltInvalid =
    ...
    (!formData.featuredImageAlt?.trim() ||
      isPlaceholderImageAltText(formData.featuredImageAlt));
      ...
      {showPreview ? (
        ...
        <div className="space-y-6">
            ...
            >
              ...
              {excerptLen > 0 && excerptLen < RECOMMENDED_EXCERPT_LENGTH ? (
                <span className="text-amber-700 dark:text-amber-500">
                  {" "}
                  · longer excerpts (around {RECOMMENDED_EXCERPT_LENGTH}+) read
                  better in listings and previews
                </span>
              ) : null}
            </p>
          </div>
          ...
          <div className="rounded-lg border border-border bg-muted/30 p-4 space-y-3">
            ...
            {internalLinkSuggestions.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No strong related posts found yet.
              </p>
            ) : (
              <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <p
              id="content-markdown-hint"
              className="text-xs text-muted-foreground"
            >
              Toolbar or shortcuts: Ctrl+B bold, Ctrl+I italic, Ctrl+` code,
              Ctrl+K link, Ctrl+Shift+I image (⌘ on Mac). Upload inserts a

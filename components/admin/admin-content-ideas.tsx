...
const CATEGORIES: ContentIdeaCategory[] = ['Work', 'Life', 'Hobbies', 'Experience']
...
interface AdminContentIdeasProps {
  onGenerateDraft: (seed: ReturnType<typeof buildIdeaGenerationSeed>, idea: ContentIdea) => void
}
...
function formFromIdea(idea: ContentIdea): IdeaFormState {
  return {
    ...
    status: idea.status,
    targetPublishAt: idea.targetPublishAt ? idea.targetPublishAt.slice(0, 16) : '',
  }
}
...
function requestBody(form: IdeaFormState) {
  return {
    ...form,
    targetPublishAt: form.targetPublishAt ? new Date(form.targetPublishAt).toISOString() : null,
  }
}
...
export function AdminContentIdeas({ onGenerateDraft }: AdminContentIdeasProps) {
  ...
  const saveIdea = async () => {
    ...
    setSaving(true)
    try {
      const url = editingId ? `/api/content-ideas/${editingId}` : '/api/content-ideas'
  ...
  const patchIdea = async (idea: ContentIdea, patch: Partial<IdeaFormState>) => {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <select
                  value={form.category}
                  onChange={(e) => setForm((prev) => ({ ...prev, category: e.target.value as ContentIdeaCategory }))}
                  className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                >
              ...
              <div className="space-y-2">
                ...
                <select
                  value={form.priority}
                  onChange={(e) => setForm((prev) => ({ ...prev, priority: e.target.value as ContentIdeaPriority }))}
                  className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                >
                  {CONTENT_IDEA_PRIORITIES.map((priority) => <option key={priority} value={priority}>{priority}</option>)}
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_220px]">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <select
                  value={form.status}
                  onChange={(e) => setForm((prev) => ({ ...prev, status: e.target.value as ContentIdeaStatus }))}
                  className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                >
                  {CONTENT_IDEA_STATUSES.map((status) => <option key={status} value={status}>{status}</option>)}
                </select>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Target publish date</label>
                <input
                  type="datetime-local"
                  value={form.targetPublishAt}
                  onChange={(e) => setForm((prev) => ({ ...prev, targetPublishAt: e.target.value }))}
                  className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                />
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={saveIdea} disabled={saving}>{saving ? 'Saving...' : editingId ? 'Update idea' : 'Add idea'}</Button>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          sortedIdeas.map((idea) => (
            <Card key={idea.id} className={idea.status === 'archived' ? 'opacity-70' : undefined}>
              <CardContent className="p-4">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="min-w-0 flex-1 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="font-semibold text-foreground">{idea.title}</h3>
                      <span className="rounded-full bg-secondary px-2 py-0.5 text-xs text-muted-foreground">{idea.category}</span>
                      <span className="rounded-full bg-secondary px-2 py-0.5 text-xs text-muted-foreground">{idea.priority}</span>
                      <span className="rounded-full bg-secondary px-2 py-0.5 text-xs text-muted-foreground">{idea.status}</span>
                    </div>
                    {idea.notes && <p className="text-sm text-muted-foreground">{idea.notes}</p>}
                    <p className="text-xs text-muted-foreground">Target: {formatDate(idea.targetPublishAt)}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button size="sm" variant="outline" onClick={() => startGenerate(idea)} disabled={idea.status === 'archived'}>
                      <Sparkles className="mr-2 h-4 w-4" />
                      Generate draft
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => startEdit(idea)}>
                      <Pencil className="mr-2 h-4 w-4" />
                      Edit
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => archiveIdea(idea)} disabled={idea.status === 'archived'}>
                      <Archive className="mr-2 h-4 w-4" />
                      Archive
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => deleteIdea(idea)}>

...
type QualityFilter =
  "attention" | "blockers" | "internal-links" | "adsense" | "healthy";
...
function hasInternalLinkIssue(audit: ContentQualityAudit): boolean {
  return audit.suggestions.some((issue) =>
    issue.toLowerCase().includes("internal link"),
  );
}
...
function hasFeaturedAltIssue(audit: ContentQualityAudit): boolean {
  return audit.blockers.some((issue) => {
    ...
    return (
      normalized.includes("featured image") && normalized.includes("alt text")
    );
  });
}
...
function filterAudits(
  ...
  if (filter === "blockers")
    return attention.filter((audit) => audit.blockers.length > 0);
  if (filter === "internal-links")
    return attention.filter(hasInternalLinkIssue);
...
export function AdminContentQuality({
  ...
  const filterOptions: Array<{
  ...
  }> = [
    {
      ...
      count: getPostsNeedingAttention(audits, audits.length).length,
    },
    {
      value: "blockers",
      label: "Blockers",
      count: audits.filter((audit) => audit.blockers.length > 0).length,
    },
    {
      value: "internal-links",
      label: "Missing links",
      count: audits.filter(hasInternalLinkIssue).length,
    },
    {
      value: "adsense",
      label: "AdSense risks",
      count: audits.filter(hasAdsenseReadinessIssue).length,
    },
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Find posts that need SEO basics, accessibility fixes, internal links,
          stronger excerpts, freshness review, or AdSense-readiness polish
          before promotion.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Filter by the issue you want to fix first. Results are prioritized
              by blockers, then lowest quality score.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {visibleAudits.length === 0 ? (
              <div className="rounded-lg border border-green-200 bg-green-50 p-4 text-sm text-green-800 dark:border-green-900/40 dark:bg-green-950/30 dark:text-green-300">
                No posts match this filter. Keep checking new drafts before
                promotion.
              </div>
            ) : (
              <div className="space-y-4">
                {visibleAudits.map((audit) => {
                  const linkSuggestions = buildInternalLinkSuggestions(
                    audit.post,
                    posts,
                    3,
                  );
                  const suggestedAltText = buildFeaturedImageAltText(
                    audit.post,
                  );
                  return (
                    <div
                    ...
                    >
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            <h3 className="font-medium text-foreground">
                              {audit.post.title || "Untitled"}
                            </h3>
                            <span
                              className={`rounded-full px-2 py-0.5 text-xs ${badgeClass(audit)}`}
                            >
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {audit.post.category} · {audit.post.status} ·{" "}
                            {audit.post.readTime} min read
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <p
                            className={`text-lg font-semibold ${scoreClass(audit.score)}`}
                          >
                            {audit.score}%
                          </p>
                          <Button
                      ...
                      audit.suggestions.some((issue) =>
                        issue.includes("internal link"),
                      ) ? (
                        <div className="mt-4 rounded-md border border-border bg-muted/30 p-3">
                          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                            ...
                            {linkSuggestions.map((suggestion) => (
                              ...
                              >
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    copyInternalLink(suggestion.markdown)
                                  }
                                >
                                  Copy markdown
                                </Button>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Advisory checks to improve publish quality.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-muted-foreground">
            ...
            <div className="flex gap-3">
              ...
              <p>
                Internal links: at least one related `/blog/...` link in the
                body.
              </p>
            </div>
            <div className="flex gap-3">
              ...
              <p>
                Freshness: published posts older than 180 days get a review
                suggestion.
              </p>
            </div>
            <div className="flex gap-3">
              ...
              <p>
                AdSense readiness: duplicate in-body H1s, placeholder image alt
                text, formulaic AI language, and short published posts.
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      {healthyPosts.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Posts scoring 90% or higher with no blockers.
            </CardDescription>
          </CardHeader>
          <CardContent>

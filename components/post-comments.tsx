...
export function PostComments({ slug }: { slug: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!vid) {
      setError(
        "Comments require browser storage. Enable cookies/storage and try again.",
      );
      return;
    }
  ...
  return (
    ...
    >
        ...
        <ul className="space-y-6 mb-10 list-none m-0 p-0">
          {comments.length === 0 ? (
            ...
            comments.map((c) => (
              <li
                key={c.id}
                className="text-sm border-b border-border/40 pb-4 last:border-0"
              >
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
        ...
        {notice && (
          <p className="text-sm text-green-700 dark:text-green-500">{notice}</p>
        )}
        {error && comments.length > 0 && (
          <p className="text-sm text-destructive">{error}</p>
        )}

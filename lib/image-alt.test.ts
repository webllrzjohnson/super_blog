...
describe('getSafeImageAltText', () => {
  it('uses explicit descriptive alt text', () => {
    expect(getSafeImageAltText('A sump pump alarm panel in a building hallway', 'Sump pump post')).toBe(
      'A sump pump alarm panel in a building hallway',
    )
  })

  it('falls back to the title when alt text is missing', () => {
    expect(getSafeImageAltText('   ', 'Failed Your G Test Twice?')).toBe('Failed Your G Test Twice?')
    expect(getSafeImageAltText(null, 'Failed Your G Test Twice?')).toBe('Failed Your G Test Twice?')
  })

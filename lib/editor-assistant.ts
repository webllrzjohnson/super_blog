...
export function buildInternalLinkSuggestions(
  ...
  return related
    .filter(
      (post) => !existingContent.includes(`/blog/${post.slug.toLowerCase()}`),
    )
...
export function buildDraftAssistantPrompt(
...
export function parseDraftAssistantResponse(
  raw: string,
): DraftAssistantSuggestion {
  let parsed: unknown = {};
  try {

  ...
  it('falls back to excerpt or category when title/tags are missing', () => {
    expect(
      buildFeaturedImageAltText(
        post({ title: '', tags: [], category: 'Life', excerpt: 'A quiet Toronto weekend reset after a long week.' })
      )
    ).toBe('Illustration for A quiet Toronto weekend reset after a long week, showing a life scene.')
  })

  it('removes markdown and keeps alt text under 160 characters', () => {
    const alt = buildFeaturedImageAltText(
      post({
        title: 'Why [I Did Not Overreact](https://example.com) During a Very Long Building Incident That Needed Calm Communication',

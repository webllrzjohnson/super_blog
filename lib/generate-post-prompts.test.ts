...
describe("generate post prompts", () => {
  it("keeps the default system prompts concise while preserving house style", () => {
    ...
    for (const prompt of [
      ...
      expect(prompt.toLowerCase()).toContain("image generation");
  ...
  it("keeps the user template compact and explicit about live-site cleanup rules", () => {
    ...
    expect(DEFAULT_USER_MESSAGE_TEMPLATE).toContain("concrete story snapshot");
  });

  it("instructs generated excerpts to carry scene details for featured images", () => {
    expect(DEFAULT_CLAUDE_SYSTEM_PROMPT).toContain(
      "setting, main activity/problem",
    );
    expect(DEFAULT_CLAUDE_SYSTEM_PROMPT).toContain("people or roles involved");
    expect(DEFAULT_CLAUDE_SYSTEM_PROMPT).toContain("concrete objects");
    expect(DEFAULT_CLAUDE_SYSTEM_PROMPT).toContain(
      "Do not make the excerpt a vague teaser",
    );
    expect(DEFAULT_GROQ_SYSTEM_PROMPT).toContain("setting, activity/problem");
    expect(DEFAULT_GROQ_SYSTEM_PROMPT).toContain("key visual objects");
  });
  ...
  it("builds user messages with recent posts for continuity only", () => {
    const message = buildUserMessage({
      ...
      schedule: "Immediate",
      recentPosts: [
        { title: "This Isn’t a Landfill", excerpt: "A compactor room story." },
      ],
    });

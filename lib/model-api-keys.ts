...
export function isModelApiKeyProvider(
  value: string,
): value is ModelApiKeyProvider {
...
export async function getAvailableModelApiKeyProviders(): Promise<
  Set<ModelApiKeyProvider>
> {
  const storedKeys = await loadStoredModelApiKeys();
  return getAvailableApiKeyProviders(storedKeys);
}

export function getAvailableApiKeyProviders(
  keys: ModelApiKeyMap,
): Set<ModelApiKeyProvider> {
  return new Set(
    MODEL_API_KEY_PROVIDERS.filter((provider) =>
      Boolean(
        process.env[getEnvApiKeyName(provider)]?.trim() || keys[provider],
      ),
    ),
  );
}

export function getAvailableAiTextProviders(
  keys: ModelApiKeyMap,
): Set<"claude" | "openai" | "groq"> {
...
export async function getAvailableConfiguredAiTextProviders(): Promise<
  Set<"claude" | "openai" | "groq">
> {
...
export async function getModelApiKeyStatus() {
  ...
  return MODEL_API_KEY_PROVIDERS.map((provider) => ({
    ...
    masked: maskApiKey(
      process.env[getEnvApiKeyName(provider)] || storedKeys[provider],
    ),
  }));
}

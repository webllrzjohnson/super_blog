...
function basePost(overrides: Partial<Post>): Post {
  return {
    ...
    slug: "slug",
    excerpt:
      "A reasonable excerpt for the post that is long enough to avoid warnings.",
...
describe("evaluatePublishChecklist", () => {
  ...
  it("returns error when body is too short", () => {
    ...
    expect(
      r.errors.some((e) => e.includes(`${MIN_PUBLISH_CONTENT_LENGTH}`)),
    ).toBe(true);
  });

  it("warns when excerpt is short", () => {
    const r = evaluatePublishChecklist(basePost({ excerpt: "tiny" }));
    expect(r.warnings.some((w) => w.toLowerCase().includes("excerpt"))).toBe(
      true,
    );
  });
  ...
  it("errors when featured image alt is placeholder text", () => {
    const r = evaluatePublishChecklist(
      basePost({
        ...
        featuredImageAlt: "Describe this image",
      }),
    );
    expect(r.errors.some((e) => e.toLowerCase().includes("alt"))).toBe(true);
  });
  ...
  it("warns when title is very short", () => {
    ...
    expect(r.warnings.some((w) => w.toLowerCase().includes("title"))).toBe(
      true,
    );
  });

  it("warns when markdown images omit alt text", () => {
    const content = `${"x".repeat(MIN_PUBLISH_CONTENT_LENGTH)}\n\n![](https://example.com/a.png)`;
...
describe("contentMayContainAffiliateLinks", () => {
  it("detects amazon URLs", () => {
    expect(
      contentMayContainAffiliateLinks("See https://amazon.com/dp/foo"),
    ).toBe(true);
    expect(contentMayContainAffiliateLinks("[buy](https://amzn.to/abc)")).toBe(
      true,
    );
  });
  ...
  it("returns false for plain prose", () => {
    expect(
      contentMayContainAffiliateLinks("Just thoughts about nature and code."),
    ).toBe(false);
  });
});

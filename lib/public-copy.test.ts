...
describe("public site identity and current monetization copy", () => {
  ...
  it('uses shared site and author identity defaults', async () => {
    const [settingsSource, postsSource, appearanceSource, feedSource] = await Promise.all([
      readFile(path.join(process.cwd(), 'lib/settings.ts'), 'utf8'),
      readFile(path.join(process.cwd(), 'lib/posts.ts'), 'utf8'),
      readFile(path.join(process.cwd(), 'components/admin/settings-appearance.tsx'), 'utf8'),
    ...
    expect(appearanceSource).toContain('siteName: initialBranding?.siteName ?? SITE_NAME')
    expect(appearanceSource).toContain('displayName: initialBranding?.displayName ?? AUTHOR_NAME')
    ...
    expect(appearanceSource).not.toContain("siteName: initialBranding?.siteName ?? 'Lester J.'")
  })
  ...
  it('describes only implemented comment moderation actions', async () => {
    const [disclaimerSource, moderationSource] = await Promise.all([
      ...
      readFile(path.join(process.cwd(), 'app/api/comments/moderate/route.ts'), 'utf8'),
    ])

import { normalizeContentIdeaInput, type NormalizedContentIdeaInput } from '@/lib/content-ideas'
...
function stripLeadPrefix(value: string): string {
  return value.replace(/^\s*(?:blog\s+idea|idea|content\s+idea)\s*[:\-–—]?\s*/i, '').trim()
}
...
function parseLabelLine(line: string, hints: ParsedHints): boolean {
  const match = line.match(/^\s*(title|idea|topic|category|priority|target|date|notes?|context)\s*:\s*(.+)$/i)
...
function extractInlineHints(text: string, hints: ParsedHints): string {
  ...
  title = title.replace(/\s+#(life|work|hobbies|hobby|experience)\b/gi, (_match, raw: string) => {
    const category = CATEGORY_HINTS[String(raw).toLowerCase()]
    if (category) hints.category = category
    return ''
  })

  title = title.replace(/\s+priority\s*:\s*(low|medium|normal|high|urgent)\b/gi, (_match, raw: string) => {
  ...
  title = title.replace(/\s+target\s*:\s*([^\s].*?)(?=\s+(?:notes?|priority|#)|$)/i, (_match, raw: string) => {
...
export function parseTelegramIdeaCapture(message: string): NormalizedContentIdeaInput {

...
import {
  ...
  MIN_GENERATED_WORDS,
} from "@/lib/generated-post-quality";
...
describe("evaluateGeneratedPostQuality", () => {
  it("blocks thin generated posts", () => {
    ...
    expect(result.errors.some((error) => error.includes("too thin"))).toBe(
      true,
    );
  });

  it("uses 700 words as the minimum standard for generated posts", () => {
    const content = `## One\n\n${Array.from({ length: 698 }, (_, index) => `word${index}`).join(" ")}`;

    const result = evaluateGeneratedPostQuality(meta, content);

    expect(MIN_GENERATED_WORDS).toBe(700);
    expect(result.wordCount).toBe(699);
    expect(result.errors).toContain(
      "Generated content is too thin (699 words). Minimum is 700.",
    );
  });
  ...
  it("blocks AI disclaimers", () => {
    const result = evaluateGeneratedPostQuality(
      meta,
      `${longContent()} As an AI language model, I cannot provide real experience.`,
    );

    expect(result.errors.some((error) => error.includes("AI disclaimer"))).toBe(
      true,
    );
  });
  ...
  it("blocks public employer naming", () => {
    const result = evaluateGeneratedPostQuality(
      meta,
      `${longContent()} Toronto Community Housing was named in this draft.`,
    );

    expect(result.errors.some((error) => error.includes("employer"))).toBe(
      true,
    );
  });
  ...
  it("blocks em dashes before save", () => {
    ...
    expect(result.errors.some((error) => error.includes("em dashes"))).toBe(
      true,
    );
  });
  ...
  it("blocks duplicate opening markdown H1s before save", () => {
    ...
    expect(result.errors.some((error) => error.includes("markdown H1"))).toBe(
      true,
    );
  });
    ...
    const result = evaluateGeneratedPostQuality(
      {
        ...
        excerpt:
          "Here is what a jam actually matters for in daily building operations.",
      },
      `${longContent()} In conclusion, it is important to note that teams should ensure the process is seamless.`,
    );

    expect(result.errors).toHaveLength(0);
    expect(result.warnings.some((warning) => warning.includes("generic"))).toBe(
      true,
    );
  });
});

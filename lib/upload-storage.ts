...
export function resolveUploadDir(
  options: ResolveUploadDirOptions = {},
): string {
...
function publicOriginFromForwardedHeaders(
  host?: string | null,
  proto?: string | null,
): string {
...
export function publicUploadUrl(
  ...
  const opts =
    typeof options === "string" ? { requestOrigin: options } : options;
  const base =
    cleanBaseUrl(opts.siteUrl ?? process.env.NEXT_PUBLIC_SITE_URL) ||
    cleanBaseUrl(
      publicOriginFromForwardedHeaders(opts.forwardedHost, opts.forwardedProto),
    ) ||

...
export function normalizeAdSenseSlotId(
  ...
  if (!normalized || !ADSENSE_SLOT_ID_PATTERN.test(normalized))
    return undefined;
  return normalized;
}

export function hasUniqueAdPositions(
  slots: readonly { position: string }[],
): boolean {
  return new Set(slots.map(({ position }) => position)).size === slots.length;
}

export function normalizeAdSenseClientId(
  value: string | null | undefined,
): string | undefined {
  const normalized = value?.trim();
  if (!normalized || !ADSENSE_CLIENT_ID_PATTERN.test(normalized))
    return undefined;
  return normalized;
}
...
export function createAdsTxtResponse(
  value: string | null | undefined,
): Response {

...
export const CONTENT_IDEA_STATUSES = ['idea', 'planned', 'generated', 'published', 'archived'] as const
...
const CATEGORIES: ContentIdeaCategory[] = ['Life', 'Work', 'Hobbies', 'Experience']
...
function isCategory(value: unknown): value is ContentIdeaCategory {
  return typeof value === 'string' && CATEGORIES.includes(value as ContentIdeaCategory)
}

function isPriority(value: unknown): value is ContentIdeaPriority {
  return typeof value === 'string' && CONTENT_IDEA_PRIORITIES.includes(value as ContentIdeaPriority)
}

function isStatus(value: unknown): value is ContentIdeaStatus {
  return typeof value === 'string' && CONTENT_IDEA_STATUSES.includes(value as ContentIdeaStatus)
}
...
export function normalizeContentIdeaInput(input: ContentIdeaInput): NormalizedContentIdeaInput {
  return {
...
export function buildIdeaGenerationSeed(idea: ContentIdea) {
  ...
  if (idea.targetPublishAt) {
    metadata.push(`Target publish date: ${new Intl.DateTimeFormat('en-US', {
      month: 'short',
      day: 'numeric',
      year: 'numeric',
      timeZone: 'UTC',

...
describe("parseTelegramIdeaCapture", () => {
  it("parses a simple blog idea message with defaults", () => {
    expect(
      parseTelegramIdeaCapture(
        "Blog idea: elevator outage communication lesson",
      ),
    ).toEqual({
  ...
  it("parses category, priority, target date, and notes from labelled lines", () => {
    expect(
      parseTelegramIdeaCapture(`Blog idea: Basement flooding morning routine
Category: Work
Priority: high
Target: 2026-08-01
Notes: Rain, old townhomes, paperwork, technician follow-up.`),
  ...
  it("handles compact hashtag-like hints without keeping them in the title", () => {
    expect(
      parseTelegramIdeaCapture(
        "idea: scooters in the hallway #life priority: low notes: weekend angle",
      ),
    ).toEqual({
  ...
  it("rejects messages without a usable title", () => {
    expect(() => parseTelegramIdeaCapture("Blog idea:   ")).toThrow(

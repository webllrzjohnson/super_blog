...
async function fetchPostSummaries(): Promise<PostListItem[]> {
  ...
  try {
    ...
    if (!rows || rows.length === 0) return [];
    return rows.map((row) =>
      mapRowToPostSummary(row as Record<string, unknown>),
    );
  } catch (err) {
...
const getPostSummariesCached = unstable_cache(
  fetchPostSummaries,
  ["posts-summaries"],
  {
    tags: [CACHE_TAG_POSTS],
...
export async function savePostToDb(post: Post): Promise<Post | null> {
  ...
  try {
    ...
    const result =
      await sql`INSERT INTO posts ${sql(row)} ON CONFLICT (id) DO UPDATE SET ${sql(row)} RETURNING *`;
    return result[0]
      ? mapRowToPost(result[0] as Record<string, unknown>)
      : null;
  } catch (err) {

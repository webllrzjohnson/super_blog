...
async function fetchRecentPublishedPosts(): Promise<
...
async function resolveFeaturedImage(
  ...
  const alt = buildPostImageAlt(imageSource);
  ...
  if (!apiKey) {
    return {
      alt,
      error: "OpenAI API key not configured for featured image generation.",
    };
  }
    ...
    const imageRes = await fetch(
      "https://api.openai.com/v1/images/generations",
      {
        ...
        body: JSON.stringify({
          ...
          prompt: buildPostImagePrompt(imageSource, ai.imagePromptTemplate),
          size: "1536x1024",
        }),
      },
    );
...
async function generateRawPostText(
  ...
  const attemptedLabels = providers
...
async function buildGeneratedPostDraft(
  ...
  try {
    ...
    const generated = await generateRawPostText(ai, {
      topic,
      context,
      schedule,
      recentPosts,
    ...
    const slug =
      sanitizeSlug(parsed.meta.slug) || sanitizeSlug(parsed.meta.title);
    if (!slug) {
    ...
    if (allPosts.some((p) => p.slug.trim().toLowerCase() === slug)) {
      return {
        ok: false,
        message: "A post with this slug already exists. Try a different topic.",
      };
    }
    ...
    const imageSource =
      parsed.meta.excerpt.trim() || parsed.meta.title.trim() || topic;
    const image = options.includeFeaturedImage
      ? await resolveFeaturedImage(imageSource, ai, params.featuredImage)
      : {
          url: undefined,
          alt: buildPostImageAlt(imageSource),
          error: undefined,
        };
...
export async function generateAndSavePost(
  ...
  const draft = await buildGeneratedPostDraft(params, {
  ...
  if (!saved) {
    return {
      ok: false,
      message: "Failed to save post. Ensure the database is configured.",
    };
  }

...
export async function getContentIdeasFromDb(): Promise<ContentIdea[]> {
  ...
  try {
    ...
    `
    return sortContentIdeas(rows.map((row) => mapRowToContentIdea(row as Record<string, unknown>)))
  } catch (err) {
...
export async function createContentIdeaInDb(input: ContentIdeaInput): Promise<ContentIdea | null> {
  ...
  try {
    const rows = await sql`
      ...
      RETURNING id, title, notes, category, priority, status,
                target_publish_at, generated_post_id, created_at, updated_at, archived_at
    `
    return rows[0] ? mapRowToContentIdea(rows[0] as Record<string, unknown>) : null
  } catch (err) {
...
export async function updateContentIdeaInDb(
  ...
  const generatedPostId =
    typeof input.generatedPostId === 'string' && input.generatedPostId.trim()
      ...
      : null
  const archivedAt = normalized.status === 'archived' ? new Date().toISOString() : null

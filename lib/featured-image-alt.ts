...
function truncateSentence(value: string, maxLength: number): string {
  ...
  return `${cleaned.slice(0, maxLength - 1).trimEnd().replace(/[,.:-]+$/, '')}.`
}

export function buildFeaturedImageAltText(post: Pick<Post, 'title' | 'excerpt' | 'category' | 'tags'>): string {
  const subject = cleanText(post.title) || firstSentence(post.excerpt) || post.category
  ...
  return truncateSentence(`Illustration for ${subject}, showing a ${scene}${about}.`, 160)
}
...
export function buildFeaturedImageAltTextFromTopic(topic: string): string {
  ...
  return truncateSentence(`Illustration for ${subject}, showing a Toronto apartment building scene.`, 160)
}

...
export function validateResourceFile(file: File): void {
  if (!ALLOWED_RESOURCE_TYPES[file.type]) {
    throw new Error(
      "Unsupported file type. Upload PDF, Word, Excel, TXT, CSV, or RTF files.",
    );
  }
...
export async function getResourceFilesFromDb(
  ...
  return options.includeUnpublished
    ? files
    : files.filter((file) => file.published);
}
...
export async function updateResourceFileInDb(
  id: string,
  patch: Partial<
    Pick<ResourceFileItem, "title" | "description" | "category" | "published">
  >,

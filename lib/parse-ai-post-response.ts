...
export function parseAiPostResponse(raw: string): ParsedAiPostResponse {
  ...
  const result = GeneratedPostMetaSchema.safeParse(
    normalizeGeneratedPostMeta(parsedJson),
  );
  if (!result.success) {
    throw new Error(
      "AI response metadata did not match the expected post format",
    );
  }
...
export function normalizeGeneratedPostMeta(input: unknown): unknown {
  ...
  if (typeof obj.category === "string") {
    ...
    if (
      !POST_CATEGORIES.includes(trimmed as (typeof POST_CATEGORIES)[number])
    ) {
      obj.category = "Life";
    }
  }

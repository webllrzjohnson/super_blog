...
describe("getAdjacentPosts", () => {
  it("returns newer and older in published chronological order", () => {
    ...
    const { prev: older, next: newer } = getAdjacentPosts(pMid, [
      pNew,
      pMid,
      pOld,
    ]);
...
describe("getPostsByTag", () => {
  it("matches normalized tags case-insensitively", () => {
    const posts = [
      mockPost({ id: "1", tags: ["Building Operations"] }),
      mockPost({ id: "2", tags: ["running"] }),
    ];

    expect(
      getPostsByTag(posts, "building operations").map((post) => post.id),
    ).toEqual(["1"]);
    expect(
      getPostsByTag(posts, "BUILDING OPERATIONS").map((post) => post.id),
    ).toEqual(["1"]);

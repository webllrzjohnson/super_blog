...
describe('content quality audit', () => {
  it('marks a complete post as healthy', () => {
    const result = auditContentQuality(post({ featuredImage: '/uploads/outage.jpg', featuredImageAlt: 'Building hallway during outage response' }))
  ...
  it('flags missing SEO and discovery basics', () => {
    ...
    expect(result.warnings).toContain('Excerpt is short; expand it for search and social previews.')
    expect(result.warnings).toContain('No tags are set.')
    expect(result.suggestions).toContain('Add at least one internal link to a related post.')
    expect(result.suggestions).toContain('Post is thin; expand it with more detail before promoting it.')
  ...
  it('flags stale published posts that have not been updated recently', () => {
    ...
    expect(result.suggestions).toContain('Published post is over 180 days old; review for freshness.')
  })

  it('summarizes quality stats and top attention items', () => {
    const posts = [
      post({ id: 'healthy', featuredImage: '/img.jpg', featuredImageAlt: 'Alt text' }),
      post({
        id: 'bad',
        title: 'Bad',
        excerpt: '',

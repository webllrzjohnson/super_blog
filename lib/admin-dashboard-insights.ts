...
export type CalendarPost = Pick<Post, 'id' | 'title' | 'slug' | 'status' | 'publishedAt' | 'category'>
...
export function getUpcomingPosts(posts: Post[], now: Date = new Date(), limit = 5): Post[] {
  const nowTime = now.getTime()
  return posts
...
export function getAdminActionItems(
  ...
  if (pendingComments > 0) {
    items.push(`${pendingComments} comment${pendingComments === 1 ? '' : 's'} need moderation.`)
  }
  if (stats.overdueScheduled > 0) {
    items.push(`${stats.overdueScheduled} scheduled post${stats.overdueScheduled === 1 ? '' : 's'} are overdue and should be reviewed.`)
  }
  if (stats.drafts === 0) {
    items.push('Draft queue is empty — generate or write the next post idea.')
  }
  if (stats.scheduled === 0) {
    items.push('No posts are scheduled. Add at least one future post to keep publishing consistent.')
  }

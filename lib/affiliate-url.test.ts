...
describe("isLikelyAffiliateUrl", () => {
  it("detects Amazon and short links", () => {
    ...
    expect(isLikelyAffiliateUrl("https://www.amazon.ca/gp/product/x")).toBe(
      true,
    );
...
describe("isExternalHref", () => {
  it("treats same host as internal when site URL matches", () => {
    ...
    expect(isExternalHref("https://mysite.com/x", "https://mysite.com")).toBe(
      false,
    );
  });

  it("treats other hosts as external", () => {
    expect(isExternalHref("https://other.com", "https://mysite.com")).toBe(
      true,
    );
  });
});

...
export async function POST(request: Request) {
  ...
  try {
    ...
    const item = await saveUploadedResourceFile({ file, title, description, category, published })
    return NextResponse.json({ file: item })
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to save resource file' },
      { status: 400 }
    )
  }
}
...
export async function PATCH(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: 'Invalid file update', details: parsed.error.flatten() }, { status: 400 })
  }
...
export async function DELETE(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: 'Invalid delete request', details: parsed.error.flatten() }, { status: 400 })
  }

...
if (!message) {
  console.error(
    'Usage: node scripts/capture-telegram-idea.mjs "Blog idea: ..."',
  );
  process.exit(1);
}
...
console.log(
  `Status: ${data.idea?.status ?? "idea"} · Priority: ${data.idea?.priority ?? "medium"} · Category: ${data.idea?.category ?? "Work"}`,
);
